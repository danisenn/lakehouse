...
def parse_args(argv: Optional[List[str]] = None) -> argparse.Namespace:
    ...
    # Lakehouse config
    ap.add_argument("--lakehouse", action="store_true", help="Use Lakehouse connection instead of local files")
    ap.add_argument("--query", type=str, default="", help="SQL query to execute (used if --schema is not set)")
    ap.add_argument("--schema", type=str, default="", help="Schema to scan for tables (e.g., lakehouse.datalake.raw)")
...
def main(argv: Optional[List[str]] = None) -> int:
    ...
    if args.lakehouse:
        if not args.query and not args.schema:
            print("[error] Either --query or --schema is required when using --lakehouse")
            return 1
        try:
            ...
            if args.schema:
                ...
                print(f"Using Lakehouse connection with schema: {args.schema}")
            else:
                source = LakehouseSQLDataSource(
                    connection_uri=conn, 
                    query=args.query
                )
                print(f"Using Lakehouse connection with query: {args.query}")
        except ImportError:
            print("[error] Could not import src.connection.connection. Ensure dependencies are installed.")
            return 1
        except Exception as e:
            print(f"[error] Failed to establish Lakehouse connection: {e}")
            return 1
    else:
