...
from .patterns import detect_value_patterns
...
class SemanticFieldMapper:
    ...
    strategy: 'auto'|'exact'|'normalized'|'fuzzy' — currently advisory; 'auto' uses all.
    ...
    def _scores_for_column(
        ...
        if series is not None and pl is not None:
            try:
                _, boosts = detect_value_patterns(series)
                # simple aggregate: take the top boost if any
                if boosts:
                    pattern_boost = max(boosts.values())
        ...
        for ref in self.reference_fields:
            ...
            score, details = aggregate_score(comps, dtype_boost=dtype_boost, pattern_boost=pattern_boost)
